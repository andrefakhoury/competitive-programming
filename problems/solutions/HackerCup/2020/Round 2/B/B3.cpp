		...
		for (int i = 0; i < n; i++) {
			...
			for (int p1 = 0; p1 < n; p1++) {
				for (int p2 = p1 + 1; p2 < n; p2++) {
					double current = prob[p1][match-1] * prob[p2][match-1];
					if (p1 != i && p2 != i) {
						wait += current;
					} else if (p1 == i) {
						lose += current;
