...
class Mo {
	...
	unordered_map<string, unordered_set<string> > sub;

	unordered_set<string> genSubs(string& s) {
		unordered_set<string> ret;

		int n = (int)s.size();
		for (int i = 0; i < n; i++)
			for (int l = 1; l <= n-i; l++)
				ret.insert(s.substr(i, l));
		return ret;
	}
...
public:
	...
	Mo(vector<string>& vec) {
		...
		for (string s : vec) sub[s] = genSubs(s);
	}
	...
	void add(int id) {
		for (string s : sub[vec[id]]) freq[s]++;
	}

	void remove(int id) {
		for (string s : sub[vec[id]]) {
			freq[s]--;	
