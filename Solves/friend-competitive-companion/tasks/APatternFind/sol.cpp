...
int main() {
	...
	while(t--) {
		...
		vector<Hash> v = Hash::calculateHashVector(a, p);
		vector<Hash> power = Hash::calculatePowerVector(p, n + 1);

		Hash h = Hash::calculateHash(b);

		vector<int> ans;
		for (int i = m - 1; i < n; i++) { // a[i-m+1..i] == b
			Hash cur = v[i];
			if (i - m + 1 > 0) cur -= v[i-m];
			if (cur == h * power[i - m + 1])
				ans.pb(i - m + 2);
		}
