...
public:
	...
	inline double dist(Point<Ty> const& p) const { return (p - *this).len(); }

	template <typename Ty>
	inline T dist2(Point<Ty> const& p) const { return (p - *this).len2(); }
	...
	inline static Point<double> minimumEnclosingCircle(vector<Point> v) {
		v = convexHull(v);
		Point<double> center(0, 0);
		int n = v.size();

		for (Point const& p : v) center.x += p.x, center.y += p.y;
		center = center / n;
		...
		while(p > EPS) {
			...
			int j = 0;
			for (int i = 1; i < n; i++) {
				double cur = v[0].dist(center);
				if (d < cur) d = cur, j = i;
			}

			center = center + (v[j] - center) * p;
			p *= 0.998;
		}
...
template<typename T> istream& operator>>(istream& in, Point<T> const& p) { return in >> p.x >> p.y; }
...
int main() {
	...
	cout << fixed << setprecision(10) << pt::minimumEnclosingCircle(v) << "\n";
}
