...
struct Circle {
	...
	// -1: dont, 0: one point, 1: two points, 2: infinite
	template<typename Tp> int do_intersect(Circle<Tp> const& a) {
		if (sign(c - a.c) == 0 && r == a.r) return 2;
		double dist = r + a.r - c.dist(a.c);
		return sign(dist);
	}
};
